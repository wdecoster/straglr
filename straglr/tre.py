...
class TREFinder:
    def __init__(
        ...
        max_num_clusters=3,
        min_cluster_size=2,
        genotype_in_size=False,
        trf_args="2 5 5 80 10 10 500 -d -h",
        debug=False,
    ):
        self.bam = bam
    ...
    def construct_trf_output(self, input_fasta):
        ...
        if m is not None:
            return "{}/{}.{}.dat".format(
                os.getcwd(), os.path.basename(input_fasta), m.group(1).replace(" ", ".")
            )
        ...
        with open(trf_output, "r") as ff:
            for line in ff:
                ...
                if cols[0] == "Sequence:":
                    ...
                    if (
                        len(cols[13]) >= self.min_str_len
                        and len(cols[13]) <= self.max_str_len
                    ):
    ...
    def extract_tres(self, ins_list, target_flank=3000):
        ...
        for ins in ins_list:
            ...
            prefix = ".".join(
                map(str, [INS.eid(ins), len(ins[5]), target_start, target_end])
            )
            trf_input += ">{}.q\n{}\n".format(prefix, ins[7])
            trf_input += ">{}.t\n{}\n".format(
                prefix,
                self.extract_genome_neighbour(
                    ins[0], ins[1], target_flank, genome_fasta
                ),
            )
            trf_input += ">{}.i\n{}\n".format(prefix, ins[5])
    ...
    def analyze_trf(self, results, target_flank, full_cov=0.7):
        ...
        for seq_id in sorted(results.keys()):
            ...
            if "t" in results[seq_id]:
                pat, pgstart, pgend = self.analyze_trf_per_seq(
                    results[seq_id],
                    int(ins_len),
                    int(gstart),
                    int(gend),
        ...
        def check_same_pats(rep1, rep2):
            if rep1 in same_pats:
                ...
                for rep in same_pats[rep1]:
                    if rep2 in rep:
                        if (
                            float(rep.count(rep2) * len(rep2)) / len(rep)
                            >= min_fraction
                        ):
                            return True
                    elif rep in rep2:
                        if (
                            float(rep2.count(rep) * len(rep)) / len(rep2)
                            >= min_fraction
                        ):
                            return True
    ...
    def combine_trf_coords(self, coords, bounds, buf=20, max_sep=50):
        ...
        coords = [
            c for c in coords if not (c[1] < bounds[0] - buf or c[0] > bounds[1] + buf)
        ]
        if not coords:
            return []
    ...
    def analyze_trf_per_seq(
        ...
        if "i" in result:
            filtered_results["i"] = [
                r for r in result["i"] if float(r[1] - r[0] + 1) / ins_len >= full_cov
            ]
        ...
        for pt in ("q", "t"):
            if pt in result:
                filtered_results[pt] = [
                    r for r in result[pt] if r[0] <= mid_pts[0] and r[1] >= mid_pts[1]
                ]
        ...
        for i_result in sorted(filtered_results["i"], key=lambda r: len(r[13])):
            ...
            for pt in ("t", "q"):
                for r in sorted(
                    filtered_results[pt], key=lambda r: len(r[13]), reverse=True
                ):
                    pat = r[13]
                    if (
                        i_pat in pat
                        or pat in i_pat
                        or self.is_same_repeat((i_pat, pat), same_pats)
                    ):
            ...
            if i_pat_matches["q"] or i_pat_matches["t"]:
                ...
                if i_pat_matches["t"]:
                    ...
                    for i in range(len(filtered_results["t"])):
                        ...
                        dist_from_mid[i] = min(
                            abs(float(result[0]) - target_flank),
                            abs(float(result[1]) - target_flank),
                        )
                    ...
                    for i in range(len(filtered_results["t"])):
                        ...
                        if len(set(r[13])) > 1 and self.is_same_repeat(
                            (i_pat, r[13]), min_fraction=1
                        ):
                    ...
                    if pgstart is None:
                        for i in range(len(filtered_results["t"])):
                            ...
                            if (
                                i_pat in r[13]
                                or r[13] in i_pat
                                or self.is_same_repeat((i_pat, r[13]), same_pats)
                            ):
                                if rep_len is None or rep_lens[i] > rep_len:
                                    pgstart, pgend = (
                                        gstart + r[0] - 1,
        ...
        if not pattern_matched and filtered_patterns["i"]:
            ...
            if filtered_results["t"]:
                tpats = [
                    r
                    for r in filtered_results["t"]
                    if int(r[0]) < target_flank and int(r[1]) > target_flank
                ]
                if tpats:
                    tpats_sorted = sorted(tpats, key=lambda p: len(p[-1]), reverse=True)
                    pgstart, pgend = (
                        gstart + tpats_sorted[0][0] - 1,
                        gstart + tpats_sorted[0][1] - 1,
                    )
            if not pgstart and not pgend:
                ...
                candidates = sorted(
                    [
                        r
                        for r in filtered_results["i"]
                        if r[13] in filtered_patterns["i"]
                    ],
    ...
    def merge_loci(self, tres, d=100):
        ...
        for tre in tres:
            for motif in tre[8].split(","):
                bed_line += "{}\n".format(
                    "\t".join(map(str, [tre[0], tre[1], tre[2], motif]))
                )
    ...
    def extract_aln_tuple(self, aln, tcoord, search_direction, max_extend=200):
        ...
        if search_direction == "left":
            while not found and tpos >= tcoord - max_extend:
                found = [
                    p for p in aln.aligned_pairs if p[1] == tpos and p[0] is not None
                ]
                tpos -= 1
        else:
            while not found and tpos <= tcoord + max_extend:
    ...
    def extract_subseq(
        self, aln, target_start, target_end, reads_fasta=None, max_extend=50
    ):
        tstart = None
        tend = None
        qstart = None
        ...
        if target_start >= aln.reference_start and target_start <= aln.reference_end:
            aln_tuple = self.extract_aln_tuple(
                aln, target_start, "left", max_extend=max_extend
            )
            if aln_tuple:
        ...
        if target_end >= aln.reference_start and target_end <= aln.reference_end:
            aln_tuple = self.extract_aln_tuple(
                aln, target_end, "right", max_extend=max_extend
            )
            if aln_tuple:
                qend, tend = aln_tuple
        ...
        if qstart is not None and qend is not None:
            ...
            if not reads_fasta:
                ...
                seq = INSFinder.get_seq(
                    reads_fasta, aln.query_name, aln.is_reverse, [qstart, qend]
                )
    ...
    def examine_repeats(self, seq, repeat, max_sep=100, min_cov=0.8):
        ...
        return (
            float(sorted_coords[-1] - sorted_coords[0] + 1) / len(seq) >= min_cov,
    ...
    def find_similar_long_patterns_gt(self, results, patterns, min_len=15, word_size=4):
        ...
        for seq in results.keys():
            ...
            targets[locus] |= set(
                [s for s in patterns[seq].split(",") if len(s) >= word_size]
            )
            for result in results[seq]:
                if len(result[13]) >= min_len or (
                    seq_len - 2 * self.trf_flank_size < 50
                    and len(patterns[seq]) >= 6
                    and len(result[13]) >= 0.5 * len(patterns[seq])
                ):
    ...
    def run_blastn_for_missed_clipped(self, query_fa, target_fa, word_size):
        ...
        cmd = " ".join(
            [
                ...
                "-subject",
                target_file,
                "-task blastn -word_size {} -evalue 1e-10 -outfmt 6 -out".format(
                    word_size
                ),
                blastn_out,
            ]
    ...
    def align_patterns(
        self, queries, targets, locus=None, word_size=4, min_word_size=4
    ):
        query_fa = ""
        min_len = None
        for seq in queries:
        ...
        if query_fa and target_fa:
            ...
            cmd = " ".join(
                [
                    ...
                    "-subject",
                    target_file,
                    "-task blastn -word_size {} -outfmt 6 -perc_identity 80 -qcov_hsp_perc 80 -out".format(
                        word_size
                    ),
                    blastn_out,
                ]
    ...
    def extract_alleles_trf(
        ...
        for seq in results.keys():
            ...
            for result in results[seq]:
                if (
                    len(set(result[13])) > 1
                    and len(result[13]) >= self.min_str_len
                    and len(result[13]) <= self.max_str_len
                ):
                    for pat in expected_pats:
                        if self.is_same_repeat(
                            (result[13], pat), same_pats=same_pats[locus]
                        ):
            ...
            if results_matched:
                ...
                combined_coords = self.combine_trf_coords(
                    [(r[0], r[1]) for r in results_matched], bounds
                )
                ...
                if combined_coords:
                    ...
                    if combined_coords[0][0] >= (
                        bounds[0] + too_far_from_read_end
                    ) or combined_coords[0][1] <= (bounds[1] - too_far_from_read_end):
                        if self.debug:
                            print(
                                "too_far_from_read_end",
                                locus,
                                read,
                                combined_coords[0][0],
                                combined_coords[0][1],
    ...
    def extract_alleles_regex(
        self, headers, repeat_seqs, flank, clipped, bam, strands, patterns, reads_fasta
    ):
        alleles = defaultdict(dict)
    ...
    def extract_missed_clipped(
        self, aln, clipped_end, gpos, min_proportion=0.4, reads_fasta=None
    ):
        clipped_size = None
        if (
            clipped_end == "start"
            and aln.cigartuples[0][0] >= 4
            and aln.cigartuples[0][0] <= 5
        ):
            clipped_size = aln.cigartuples[0][1]
        elif (
            clipped_end == "end"
            and aln.cigartuples[-1][0] >= 4
            and aln.cigartuples[-1][0] <= 5
        ):
        ...
        if clipped_size is not None:
            if clipped_end == "start":
                ...
                tup = self.extract_aln_tuple(
                    aln, min(aln.reference_start, gpos[0]) - self.trf_flank_size, "left"
                )
                if tup:
                    qstart, qend = 0, tup[0]
                    tpos = tup[1]
                ...
                qstart, qend = (
                    aln.query_alignment_end - self.trf_flank_size,
                    aln.infer_read_length(),
                )
                tup = self.extract_aln_tuple(
                    aln, max(aln.reference_end, gpos[1]) + self.trf_flank_size, "right"
                )
                if tup:
    ...
    def parse_blastn(self, blastn_out):
        ...
        with open(blastn_out, "r") as ff:
            for line in ff:
                ...
                results.append(
                    [query, subject, pid, alen, evalue, qstart, qend, sstart, send]
                )
    ...
    def rescue_missed_clipped(
        self, missed, genome_fasta, min_mapped=0.7, max_evalue=1e-10
    ):
        rescued = []
        target_fa = ""
        query_fa = ""
        ...
        for locus, clipped_end, read, qstart, qend, tpos, seq in missed:
            target_fa += ">{}:{}:{}:{}:{}\n{}\n".format(
                read, qstart, qend, tpos, len(seq), seq
            )
            seqs[read] = seq
            loci[read] = locus
            ...
            query_fa += ">{}:{}:{}:{}:{}\n{}\n".format(
                read, clipped_end, pstart, pend, len(pseq), pseq
            )
        ...
        if os.path.exists(blastn_out):
            ...
            if results:
                ...
                for read in sorted(by_read.keys()):
                    ...
                    filtered_results = [
                        r
                        for r in by_read[read]
                        if r[3] / rlen >= min_mapped and r[4] <= max_evalue
                    ]
    ...
    def get_alleles(self, loci, reads_fasta=[], closeness_to_end=50):
        bam = pysam.AlignmentFile(self.bam, "rc", reference_filename=self.genome_fasta)
        if self.reads_fasta:
        ...
        for locus in loci:
            ...
            for aln in bam.fetch(
                ...
                if self.check_split_alignments and (
                    (
                        aln.reference_start >= locus[1] - split_neighbour_size
                        and aln.reference_start <= locus[2] + split_neighbour_size
                    )
                    or (
                        aln.reference_end >= locus[1] - split_neighbour_size
                        and aln.reference_end <= locus[2] + split_neighbour_size
                    )
                ):
                    check_end = None
                    start_olap = False
                    ...
                    if check_end is not None:
                        clipped_end, partner_start = (
                            INSFinder.is_split_aln_potential_ins(
                                aln,
                                min_split_size=400,
                                closeness_to_end=10000,
                                check_end=check_end,
                                use_sa=True,
                            )
                        )
            ...
            for read in clipped.keys():
                if len(clipped[read].keys()) == 2:
                    ...
                    if (
                        reads_fasta
                        or aln1.query_alignment_end < aln2.query_alignment_start
                    ):
                        aln1_tuple = self.extract_aln_tuple(
                            aln1, locus[1] - self.trf_flank_size, "left"
                        )
                        aln2_tuple = self.extract_aln_tuple(
                            aln2, locus[2] + self.trf_flank_size, "right"
                        )
                        qstart = None
                        qend = None
                        tstart = None
                        ...
                        if aln1_tuple and aln2_tuple:
                            ...
                            if not seq:
                                if self.debug:
                                    print(
                                        "problem getting seq2 {} {}".format(
                                            aln.query_name, locus
                                        )
                                    )
                                continue
                        else:
                            if (
                                aln1.query_alignment_length
                                > aln2.query_alignment_length
                            ):
                                clipped_end = "end"
                                aln = aln1
                            else:
                                clipped_end = "start"
                                aln = aln2
                            missed = self.extract_missed_clipped(
                                aln, clipped_end, locus[1:], reads_fasta=reads_fasta
                            )
                            if missed:
                                qstart, qend, tpos, seq = missed
                                missed_clipped.append(
                                    [
                                        tuple(locus),
                                        clipped_end,
                                        read,
                        ...
                        header, fa_entry = self.create_trf_fasta(
                            locus[:3],
                            aln1.query_name,
                            tstart,
                            tend,
                    ...
                    if missed:
                        qstart, qend, tpos, seq = missed
                        missed_clipped.append(
                            [tuple(locus), clipped_end, read, qstart, qend, tpos, seq]
                        )
            ...
            for aln in alns:
                if (
                    aln.reference_start <= locus[1] - single_neighbour_size
                    and aln.reference_end >= locus[2] + single_neighbour_size
                ):
                    # don't consider alignment if it's deemed split at locus
                    if aln.query_name in clipped:
                        continue
                    seq, tstart, tend, qstart = self.extract_subseq(
                        aln,
                        locus[1] - self.trf_flank_size,
                        locus[2] + self.trf_flank_size,
                        reads_fasta=reads_fasta,
                    )
                    if seq is None:
                        if self.debug:
                            print(
                                "problem getting seq1 {} {} {} {} {}".format(
                                    aln.query_name, locus, tstart, tend, qstart
                                )
                            )
                        continue
        ...
        if missed_clipped:
            ...
            for read, clipped_end, qstart, qend, tstart, tend, seq, locus in rescued:
                ...
                if not (aln.reference_start + closeness_to_end) <= locus[1] and not (
                    aln.reference_end - closeness_to_end >= locus[2]
                ):
                    continue
                header, fa_entry = self.create_trf_fasta(
                    locus[:3], read, tstart, tend, qstart, seq, aln.infer_read_length()
                )
                patterns[header] = locus[-3]
                repeat_seqs[header] = seq
    ...
    def examine_ins(self, ins_list, min_expansion=0):
        def filter_tres(ins_list, tre_events):
            for ins in ins_list:
                if ins[6] == "tre":
                    motifs = [
                        m
                        for m in ins[-1].split(",")
                        if len(m) >= self.min_str_len and len(m) <= self.max_str_len
                    ]
                    if motifs:
        ...
        if self.nprocs > 1:
            ...
            expansions = combine_batch_results(
                batched_results, type(batched_results[0])
            )
        else:
        ...
        if tre_events:
            ...
            if variants:
                ...
                return [
                    v
                    for v in variants
                    if Variant.above_min_expansion(v, min_expansion, self.min_support)
                ]
    ...
    def collect_alleles(self, loci):
        ...
        if self.nprocs > 1:
            ...
            tre_variants = combine_batch_results(
                batched_results, type(batched_results[0])
            )
        else:
    ...
    def output_vcf(self, variants, out_file, sample, loci_bed, genome_fasta, bam):
        ...
        pyBAM = pysam.AlignmentFile(bam, "rc", reference_filename=genome_fasta)
        ...
        with open(loci_bed, "r") as ff:
            for line in ff:
                ...
                cols = line.rstrip().split()
                if len(cols) >= 6:
                    loci["{}:{}-{}".format(cols[0], cols[1], cols[2])] = (
                        cols[4],
                        cols[5],
                    )
        ...
        for variant in natsorted(variants, key=itemgetter(0, 1, 2)):
            ...
            output_vcf_header_alt.add(
                '##ALT=<ID=STR{},Description="Allele comprised of {} repeat units">\n'.format(
                    allele1_repeat_count, allele1_repeat_count
                )
            )
            ...
            if homzygous:
                ...
                if is_ref_allele:
                    ...
                    output_vcf_body += "{}\t{}\t.\t{}\t<STR{}>\t.\tPASS\tSVTYPE=DUP;END={};REF={};RL={};RU={};REPID={};VARID={}\tGT:SO:REPCN:REPCI:ADSP:ADFL:ADIR:LC\t1/1:SPANNING/SPANNING:{}/{}:{}-{}/{}-{}:{}/{}:0/0:0/0:{}\n".format(
                        ...
                        allele1_repeat_count,
                        allele1_repeat_count,
                        round(allel1_ci_lower),
                        round(allel1_ci_upper),
                        round(allel1_ci_lower),
                        round(allel1_ci_upper),
                        allel1_support / 2,
                        allel1_support / 2,
                ...
                output_vcf_header_alt.add(
                    '##ALT=<ID=STR{},Description="Allele comprised of {} repeat units">\n'.format(
                        allele2_repeat_count, allele2_repeat_count
                    )
                )
                ...
                output_vcf_body += "{}\t{}\t.\t{}\t<STR{}>,<STR{}>\t.\tPASS\tSVTYPE=DUP;END={};REF={};RL={};RU={};REPID={};VARID={}\tGT:SO:REPCN:REPCI:ADSP:ADFL:ADIR:LC\t1/2:SPANNING/SPANNING:{}/{}:{}-{}/{}-{}:{}/{}:0/0:0/0:{}\n".format(
                    ...
                    round(allel1_ci_lower),
                    round(allel1_ci_upper),
                    round(allel2_ci_lower),
                    round(allel2_ci_upper),
                    allel1_support,
                    allel2_support,
                    lc,
                )
